...
async function station_info(station_name) {
    ...
    try {
        const [data, team_data] = await Promise.all([
            fetch(`http://localhost:4011/api/station/${station_name}`).then(response => response.json()),
...
async function showLocate() {
    try {
        const response = await fetch('http://localhost:4011/api/teams');
...
async function showMap() {
    try {
        const response = await fetch('http://localhost:4011/api/collapse_status');
        const data = await response.json();
...
async function showCollapse_time() {
    try {
        const response = await fetch('http://localhost:4011/api/next_collapse_time');
        const data = await response.json();
...
async function showImprisoned() {
    const team = document.querySelector('#team').innerHTML;
    try {
        const response = await fetch(`http://localhost:4011/api/team/${team}`);
        const data = await response.json();
    ...
    return new Promise((resolve, reject) => {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(async (position) => {
                try {
                    const response = await fetch(`http://localhost:4011/api/gps_location/${team}/${position.coords.latitude}/${position.coords.longitude}`);
                    const data = await response.json();
